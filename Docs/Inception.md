...
>
> Build a secure, production-grade Docker infrastructure for WordPress hosting by following this structured implementation roadmap. At each stage, ensure you can explain your choices and configurations to peers during evaluation.
>
> ---
>
...
> - Set up a Virtual Machine (VM) with Docker and Docker Compose installed. Which Linux distribution will you use (Debian vs Alpine) and why does this choice matter for container size and security?
> - Design your docker-network architecture: What subnet will you use? How will containers discover each other by service name?
> - Plan your directory structure following the requirements (Makefile at root, srcs/ folder, requirements/ subdirectories). Create a skeleton structure before writing any Dockerfiles.
...
> - Create a `.gitignore` to exclude `.env`, `secrets/`, and any credential files from version control.
>
> **Security Planning:**
> - Generate TLS/SSL certificates for NGINX (self-signed for development). What are the certificate requirements for TLSv1.2/1.3?
> - Plan your secrets storage strategy: Will you use Docker secrets, a `secrets/` folder, or both? Document the variables you'll need: DB_ROOT_PASSWORD, DB_USER, DB_PASSWORD, WP_ADMIN credentials.
> - Choose strong, non-obvious administrator usernames that avoid 'admin', 'administrator', etc. Why is this important for security?
>
> ---
>
...
>    - **Critical:** Ensure the container runs `mysqld` as PID 1, not a wrapper script with infinite loops
>
...
>    - Command: `openssl req -x509 -nodes -days 365 -newkey rsa:2048 -keyout cert.key -out cert.crt`
...
>
> 5. **Testing Checkpoint:**
>    - Can you access `https://[your-login].42.fr` in a browser? (Expect SSL warning for self-signed cert)
...
> 7. Test container restart: `docker-compose restart` - does everything come back up automatically?
...
> - What would happen if you changed the MariaDB port in docker-compose.yml? What else would need to change?
...
> - WordPress shows database connection error (check environment variables and service dependencies)
> - SSL certificate warnings in browser (expected for self-signed certs, but verify TLS version is correct)
>
> ---
>
...
> - Configure WordPress to use Redis for object caching (WP Redis plugin or `wp-config.php` settings)
...
>
> **Remember:** The goal isn't just a working infrastructure—it's a deep understanding of containerization, networking, security, and system administration that you can explain and defend to peers.
